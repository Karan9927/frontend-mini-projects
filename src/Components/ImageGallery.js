...
const ImageGallery = () => {
  ...
  return (
    <div>
      <p className="my-10 text-lg text-center underline">Click on an image!</p>
      <div className="flex justify-center gap-5 my-10">
        <img
          alt="cat-image"
          onClick={clickHandler}
          value={selectedImage}
          className="w-64 transition-all cursor-pointer rounded-xl hover:scale-110"
      ...
      {selectedImage ? (
        <div className="flex flex-col items-center justify-center my-10">
          <p className="my-5 text-xl font-bold">Selected Image</p>
          <img
            alt="cat-image"
            className=" w-[40%] rounded-md shadow-2xl shadow-black "
            src={selectedImage}
          />
        </div>
      ) : (
